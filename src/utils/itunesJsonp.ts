...
/**
 * Fetch iTunes album artwork using JSONP to completely bypass browser CORS restrictions
 * Works seamlessly on static site hosts (GitHub Pages) and localhost without proxy servers.
 */
  ...
  return new Promise((resolve) => {
    ...
    (window as unknown as Record<string, unknown>)[callbackName] = (data: { results?: Array<{ artworkUrl100?: string }> }) => {
      ...
      if (data?.results && data.results.length > 0 && data.results[0].artworkUrl100) {
        ...
        resolve(rawUrl.replace(ITUNES_IMAGE_SIZES.DEFAULT, '300x300bb.jpg'));
      } else {
        resolve(null);
      }
    };
